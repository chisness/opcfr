...
int SuitCardAbstraction::get_bucket_internal( const Game *game,
					      ...
					      const int round ) const
{
  //hand_index_t idcs[] = new hand_index_t[2];
  //uint8_t holec[] = new uint8_t[2];
  //new hand_index_t idcs[2]();
  hand_index_t * idcs = new hand_index_t[2];
  uint8_t * holec = new uint8_t[2];
  ...
  uint8_t * all_cards = new uint8_t[5];
  std::copy(holec, holec + 2, all_cards);
  std::copy(board_cards, board_cards + 3, all_cards + 2);
